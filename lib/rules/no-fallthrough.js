...
/**
 * Checks whether a SwitchCase node is adjacent to the token after it
 * @param {ASTNode} node - The case node to check
 * @param {RuleContext} context - Rule context
 * @returns {boolean} `true` if the case is adjacent to the next token
 */
function isAdjacentToNextToken(node, context) {
    var sourceCode = context.getSourceCode();
    var nextToken = sourceCode.getTokenAfter(node);

    return nextToken && nextToken.loc.start.line <= node.loc.end.line + 1;
}
    ...
    return {
        ...
        "SwitchCase:exit": function(node) {
            ...
            if (currentCodePath.currentSegments.some(isReachable) &&
                (node.consequent.length > 0 || !isAdjacentToNextToken(node, context)) &&
